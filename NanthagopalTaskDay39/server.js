...
app.get("/student/:studentId/mentors", async (req, res) => {
  try {
    ...
    if (student.pMentor.length === 0) {
      res.status(404).send("No previous mentor found for the student.");
    } else {
      const pMentorId = student.pMentor[student.pMentor.length - 1];
      const pMentor = await Mentor.findById(pMentorId);

      res.status(200).json(pMentor);
    }
  } catch (error) {
